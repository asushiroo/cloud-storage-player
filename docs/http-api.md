...
- 当前流优先来自本地已加密分片
- 如果分片不可用，会回退到本地主机导入源文件
- 这仍然是过渡实现，不是最终的百度网盘加密分片流
...
8. 生成本地 manifest 文件并写入目标 manifest 路径
9. 尝试抽取封面
10. 更新任务状态为 `completed` 或 `failed`
...
- 现在已经做了“本地切片 + 本地 AES-256-GCM 加密 + 元数据落库 + 本地 manifest 生成”
- 当前还没做的是“百度网盘上传 / 同步 + 基于百度网盘分片的播放”

...
def test_copy_baidu_refresh_token_prefers_oauth_code_over_existing_refresh_token(
    monkeypatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("BAIDU_APP_KEY", "demo-app-key")
    ...
    base_settings = build_settings(tmp_path)
    initialize_database(base_settings)
    set_baidu_refresh_token(base_settings, "old-refresh-token")
    ...
    def fake_authorize(app_settings: Settings, *, code: str) -> None:
        assert code == "new-code"
        set_baidu_refresh_token(app_settings, "new-refresh-token")
        set_baidu_access_token(app_settings, "new-access-token", expires_in=3600)

    monkeypatch.setattr("app.services.baidu_smoke.authorize_baidu_with_code", fake_authorize)

    copied = copy_baidu_refresh_token(
        base_settings,
        targets=[target_settings],
        oauth_code="new-code",
    )

    assert copied == "new-refresh-token"
    assert get_baidu_refresh_token(base_settings) == "new-refresh-token"
    assert get_baidu_refresh_token(target_settings) == "new-refresh-token"
    assert get_baidu_access_token(target_settings) == "new-access-token"

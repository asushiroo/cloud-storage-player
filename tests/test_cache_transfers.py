...
def test_process_cache_job_uses_runtime_configured_transfer_concurrency(monkeypatch, tmp_path: Path) -> None:
    settings = build_settings(tmp_path)
    video = create_video(
        settings,
        title="Runtime Configured Cache",
        mime_type="video/mp4",
        size=2048,
        manifest_path="/apps/CloudStoragePlayer/mock/manifest.bin",
        source_path=str(tmp_path / "runtime-config-cache.mp4"),
    )
